...
### Prebuilt Images
Builds of the image are available on [Docker Hub](https://hub.docker.com/ryanwclark/traefik) and is the recommended method of installation.

```bash
docker pull ryanwclark/traefik:(imagetag)
```
...
This image relies on an [Alpine Linux](https://hub.docker.com/ryanwclark/alpine) or [Debian Linux](https://hub.docker.com/ryanwclark/debian) base image that relies on an [init system](https://github.com/just-containers/s6-overlay) for added capabilities. Outgoing SMTP capabilities are handlded via `msmtp`. Individual container performance monitoring is performed by [zabbix-agent](https://zabbix.org). Additional tools include: `bash`,`curl`,`less`,`logrotate`, `nano`,`vim`.
